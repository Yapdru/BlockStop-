...
export class TeamsIntegration extends BaseUserIntegration {
  ...
  getAuthorizationUrl(state: string): string {
    const params = new URLSearchParams({
      ...
      response_type: 'code',
      scope: 'Team.ReadBasic.All Chat.Read offline_access',
      state
    });
  ...
  async exchangeCodeForToken(code: string) {
    return this.retryWithBackoff(async () => {
      ...
      if (!response.ok) {
        await this.handleApiError(new Error(`Teams token exchange failed: ${response.statusText}`));
      }
  ...
  async refreshAccessToken() {
    if (!this.refreshToken) {
      throw this.createError('NO_REFRESH_TOKEN', 'No refresh token available for Teams');
    }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      console.error('Failed to revoke Teams token:', error);
    }
  }

  async scanForThreats(): Promise<ScanResult> {
    return this.retryWithBackoff(async () => {
      const response = await fetch(`${this.apiUrl}/me/joinedTeams`, {
        headers: { Authorization: `Bearer ${this.accessToken}` }
      });
      ...
      if (!response.ok) {
        await this.handleApiError(new Error(`Teams API error: ${response.statusText}`), [429, 503]);
      }

      const data = await response.json();
      const teams = data.value || [];

      const details = teams.slice(0, 50).map((team: any) => ({
        itemId: team.id,
      ...
      return {
        provider: ServiceProvider.TEAMS,
        timestamp: new Date(),
        itemsScanned: teams.length,
        threatsDetected: 0,
        details
      };
    });
