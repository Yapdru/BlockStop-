...
export class GmailIntegration extends BaseUserIntegration {
  private clientId: string;
  private clientSecret: string;
  private redirectUri: string;
  private readonly oauthUrl = 'https://accounts.google.com/o/oauth2/v2/auth';
  private readonly tokenUrl = 'https://oauth2.googleapis.com/token';
  private readonly apiUrl = 'https://gmail.googleapis.com/gmail/v1';
  ...
  async exchangeCodeForToken(code: string) {
    return this.retryWithBackoff(async () => {
      ...
      if (!response.ok) {
        await this.handleApiError(new Error(`OAuth token exchange failed: ${response.statusText}`), [400, 401]);
      }
  ...
  async refreshAccessToken() {
    if (!this.refreshToken) {
      throw this.createError('NO_REFRESH_TOKEN', 'No refresh token available for Gmail');
    }
  ...
  async scanForThreats(): Promise<ScanResult> {
    return this.retryWithBackoff(async () => {
      const response = await fetch(`${this.apiUrl}/users/me/messages?maxResults=100&q=label:INBOX`, {
      ...
      if (!response.ok) {
        await this.handleApiError(new Error(`Gmail API error: ${response.statusText}`), [429, 503]);
      }
      ...
      const details = messages.map((msg: any) => ({
        itemId: msg.id,
        itemName: `Gmail Message ${msg.id.slice(0, 8)}`,
